 ...
 * Registration methods must be executed relatively early into the plugin loading process, before the registry is
 * frozen.
 *
 * @author Floweynt
 * @since 1.0.0
...
public interface CustomItemRegistry {
	...
	static CustomItemRegistry getInstance() {
		return MonumentaPaperAPI.getInstance().getCustomItemRegistryAPI();
	}
	 ...
	 * @return The newly registered variant set. Call {@link ItemVariantSet#defaultVariant()} to get the variant
	 * registered.
	 * @author Floweynt
	 * @since 1.0.0
	 */
